...
; *** Gyroflow(NiYien) custom downloader messages
SetupDownloadTitle=正在下載 Gyroflow(NiYien)
SetupDownloadDescription=安裝程式正在下載應用程式套件，請稍候。
SetupMissingPackageUrl=缺少應用程式套件 URL。請提供 /PACKAGEURL=<zip_url>，或在建置 setup 時設定 PackageUrl。
SetupMissingPackageSha256=缺少應用程式套件 SHA256。請提供 /PACKAGESHA256=<zip_sha256>，或在建置 setup 時設定 PackageSha256。
SetupDownloadVerifyFailed=下載或校驗 Gyroflow(NiYien) 應用程式套件失敗。
SetupMissingPackageFile=找不到本機應用程式套件檔案。
SetupPackageFileVerifyFailed=校驗本機 Gyroflow(NiYien) 應用程式套件失敗。
