...
This inserts two QML source strings into all 23 TS files, removes the
...
def xml_escape_with_eol(text: str, eol: str) -> str:
    normalized = text.replace("\r\n", "\n").replace("\r", "\n")
    return xml_escape(normalized).replace("\n", eol)


def find_source_tag(raw: str, source: str, start: int = 0, end: int | None = None) -> tuple[int, str]:
    for eol in ("\r\n", "\n", "\r"):
        tag = f"<source>{xml_escape_with_eol(source, eol)}</source>"
        pos = raw.find(tag, start, end)
        if pos != -1:
            return pos, tag
    return -1, ""
...
def remove_message(raw: str, context: str, source: str, eol: str) -> str:
    while True:
        ...
        source_pos, _ = find_source_tag(raw, source, start, end)
        if source_pos == -1:
            return raw
...
def message_block(eol: str, filename: str, line: int, source: str, trans: str | None) -> str:
    translation = (
        ...
        if trans is None
        else f"<translation>{xml_escape_with_eol(trans, eol)}</translation>"
    )
    return (
        f"    <message>{eol}"
        f'        <location filename="{filename}" line="{line}"/>{eol}'
        f"        <source>{xml_escape_with_eol(source, eol)}</source>{eol}"
        f"        {translation}{eol}"
        f"    </message>{eol}"
    )
...
def insert_after_anchor(
    ...
    start, end = context_bounds(raw, context)
    if find_source_tag(raw, source, start, end)[0] != -1:
        return raw, False
    anchor_pos, _ = find_source_tag(raw, anchor, start, end)
    if anchor_pos == -1:
...
def patch_file(path: pathlib.Path, deep_trans: str | None, mount_trans: str | None) -> list[str]:
    ...
    raw = remove_message(raw, "RenderQueue", DEEP_SOURCE, eol)
    raw, _ = insert_after_anchor(
        raw,
        "RenderQueue",
    ...
    return [
        name
        for name, changed in (("deep", deep_changed), ("mount", raw != mount_before and mount_added))
        if changed
    ]
...
def validate_file(path: pathlib.Path, require_finished: bool) -> list[str]:
    ...
    for source in (DEEP_SOURCE, MOUNT_SOURCE):
        source_pos, tag = find_source_tag(raw, source)
        if source_pos == -1:
            errors.append(f"missing {source[:24]!r}")
        elif require_finished:
            tail = raw[source_pos + len(tag) :]
    ...
    for source in OLD_RENDERQUEUE_SOURCES:
        if find_source_tag(raw, source)[0] != -1:
