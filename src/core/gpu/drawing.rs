...
impl DrawCanvas {
    ...
    pub fn put_pixel(
        ...
        if y_inverted {
            y = match stage {
                Stage::OnInput => self.height,
                Stage::OnOutput => self.output_height,
            } as i32
                - y;
        }
        if x < 0 || y < 0 || x > w as i32 * self.scale as i32 || y > h as i32 * self.scale as i32 {
            return;
        }
        let adj = if size > 2 { size as f32 / -2.0 } else { 0.0 };
        for xstep in 0..size {
            for ystep in 0..size {
                let pos = (((y as f32 / self.scale as f32 + ystep as f32 + adj).floor()) * w as f32
                    + (x as f32 / self.scale as f32 + xstep as f32 + adj).floor())
                .round() as i32;
                if pos >= 0 && pos < self.buffer.len() as i32 {
                    self.has_any_pixels = true;
                    self.buffer[pos as usize] =
                        ((color as u8) << 3) | ((alpha as u8) << 1) | (stage as u8);
                }
            }
        }
    }
