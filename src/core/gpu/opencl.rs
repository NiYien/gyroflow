...
use ocl::core::{GlTextureTarget, ImageDescriptor, MemObjectType};
use ocl::*;
...
impl OclWrapper {
        ...
        if let Some(buffers) = buffers {
            match &buffers.input.data {
                ...
                BufferSource::DirectX11 { device, .. } => {
                    props.set_property_value(ocl::enums::ContextPropertyValue::D3d11DeviceKhr(
                        *device,
                    ));
                }
                _ => {}
        ...
        for p in Platform::list() {
                ...
                Device::list(p, Some(ocl::flags::DeviceType::new().gpu().accelerator()))
            {
                for d in devs {
                    if EXCLUSIONS
                        .iter()
                        .any(|x| d.name().unwrap_or_default().contains(x))
                    {
                        continue;
                    }
                    if i == index {
                        ...
                        *CONTEXT.write() = Some(CtxWrapper {
                            ...
                            context,
                            platform: p,
                            surface_checksum: buffers.get_checksum(),
                        });
                        return Ok(());
                    }
                    i += 1;
    ...
    pub fn initialize_context(buffers: Option<&Buffers>) -> ocl::Result<(String, String)> {
        ...
        if device.is_none() {
            ...
            'outer2: for p in Platform::list() {
                ...
                {
                    for d in devs.iter() {
                        if let Ok(ocl::core::DeviceInfoResult::Type(typ)) =
                            d.info(ocl::core::DeviceInfo::Type)
                        {
        ...
        *CONTEXT.write() = Some(CtxWrapper {
            ...
            context,
            platform,
            surface_checksum: buffers.map(|x| x.get_checksum()).unwrap_or_default(),
        });
    ...
    pub fn new(
        params: &KernelParams,
        ocl_names: (&str, &str, &str, &str),
        distortion_model: DistortionModel,
        digital_lens: Option<DistortionModel>,
        buffers: &Buffers,
        drawing_len: usize,
    ) -> ocl::Result<Self> {
        if params.height < 4 || params.output_height < 4 || params.stride < 1 {
        ...
        kernel = kernel
            .replace("LENS_MODEL_FUNCTIONS;", &lens_model_functions)
        ...
        for i in 0..31 {
            ...
            if v == 4 {
                continue;
            } // Fill with background can be different per frame
            kernel = kernel.replace(
                &format!("(params->flags & {v})"),
                if (params.flags & v) == v {
                    "true"
                } else {
                    "false"
...

        {
            ...
            if !context_initialized
                || ctx.as_ref().unwrap().surface_checksum != buffers.get_checksum()
            {
                drop(ctx);
        ...
        if let Some(ref mut ctx) = *lock {
            ...
            let in_desc = ImageDescriptor::new(
                MemObjectType::Image2d,
                buffers.input.size.0,
                buffers.input.size.1,
                1,
                1,
            ...
            let out_desc = ImageDescriptor::new(
                MemObjectType::Image2d,
            ...
            let mut resolve_texture =
                |buf: &BufferDescription,
                 is_in: bool,
                 ocl_queue: &mut Queue,
                 desc: ImageDescriptor,
                 _other_img: Option<&(ocl::Image<u8>, u64)>|
                 -> ocl::Result<(Buffer<u8>, Option<(ocl::Image<u8>, u64)>)> {
                    match &buf.data {
                        ...
                        BufferSource::OpenCL { queue, .. } => {
                            if !queue.is_null() {
                                let queue_core =
                                    unsafe { core::CommandQueue::from_raw_copied_ptr(*queue) };
                            ...
                            let flags = if is_in {
                                MemFlags::new().read_only().host_no_access()
                            } else {
                                MemFlags::new().read_write().host_no_access()
                            };
                            Ok((
                                Buffer::builder()
                                    .queue(ocl_queue.clone())
                                    .len(buf.size.1 * buf.size.2)
                                    .flags(flags)
                                    .build()?,
                                None,
                            ))
                        ...
                        BufferSource::OpenGL { texture, .. } => {
                            ...
                            Ok((
                                Buffer::builder()
                                    ...
                                    .flags(flags)
                                    .build()?,
                                Some((img, *texture as u64)),
                            ))
                        }
                        #[cfg(target_os = "windows")]
                        BufferSource::DirectX11 { texture, .. } => {
                            if is_in {
                                let img = Image::from_d3d11_texture2d(
                                    ocl_queue.clone(),
                                    MemFlags::new().read_only(),
                                    desc,
                                    *texture,
                                ...
                                Ok((
                                    Buffer::builder()
                                        .queue(ocl_queue.clone())
                                        .len(img.pixel_count() * params.bytes_per_pixel as usize)
                                ...
                                Ok((
                                    Buffer::builder()
                                        .queue(ocl_queue.clone())
                                        .len(
                                            img.as_ref().unwrap().0.pixel_count()
                                                * params.bytes_per_pixel as usize,
                                        )
            ...
            let (source_buffer, image_src) =
                resolve_texture(&buffers.input, true, &mut ocl_queue, in_desc, None)?;
            let (dest_buffer, image_dst) = resolve_texture(
                &buffers.output,
                false,
                &mut ocl_queue,
                out_desc,
            ...
            let buf_params = Buffer::builder()
                .queue(ocl_queue.clone())
                .flags(flags)
                .len(std::mem::size_of::<KernelParams>())
                .build()?;
            let buf_drawing = Buffer::builder()
                .queue(ocl_queue.clone())
                .flags(flags)
                .len(drawing_len.max(4))
                .build()?;
            let buf_matrices = Buffer::builder()
                .queue(ocl_queue.clone())
                .flags(flags)
                .len(max_matrix_count)
                .build()?;
            let buf_mesh_data = Buffer::builder()
                .queue(ocl_queue.clone())
                .flags(flags)
                .len(crate::gyro_source::splines::MAX_BUFFER_SIZE)
                .build()?;
    ...
    pub fn undistort_image(
        ...
        if self.buf_matrices.len() < matrices.len() {
            log::error!(
                "Buffer size mismatch matrices! {} vs {}",
                self.buf_matrices.len(),
                matrices.len()
            );
            return Ok(());
        }
        ...
        if let Some(ref tex) = self.image_src {
            ...
            if len != self.src.len() {
                log::error!(
                    "Buffer size mismatch image_src! {} vs {}",
                    self.src.len(),
                    len
                );
                return Ok(());
        ...
        if let Some(ref tex) = self.image_dst {
            ...
            if len != self.dst.len() {
                log::error!(
                    "Buffer size mismatch image_dst! {} vs {}",
                    self.dst.len(),
                    len
                );
                return Ok(());
        ...
        if !drawing_buffer.is_empty() {
            if self.buf_drawing.len() != drawing_buffer.len() {
                log::error!(
                    "Buffer size mismatch drawing_buffer! {} vs {}",
                    self.buf_drawing.len(),
                    drawing_buffer.len()
                );
                return Ok(());
            }
        ...
        if !itm.mesh_data.is_empty() {
            if self.buf_mesh_data.len() < itm.mesh_data.len() {
                log::error!(
                    "Buffer size mismatch buf_mesh_data! {} vs {}",
                    self.buf_mesh_data.len(),
                    itm.mesh_data.len()
                );
                return Ok(());
            }
        ...
        match buffers.input.data {
            ...
            BufferSource::Cpu { ref buffer } => {
                if self.src.len() != buffer.len() {
                    log::error!(
                        "Buffer size mismatch input! {} vs {}",
                        self.src.len(),
                        buffer.len()
                    );
                    return Ok(());
            ...
            BufferSource::OpenCL { texture, .. } => unsafe {
                if buffers.input.texture_copy {
                    ...
                    use ocl::core::AsMem;
                    ocl::ffi::clEnqueueCopyBuffer(
                        self.queue.as_ptr(),
                        texture,
                        self.src.as_mem().as_ptr(),
                        0,
                        0,
                        len,
                        0,
                    ...
                    self.kernel.set_arg_unchecked(
                        0,
                        core::ArgVal::from_raw(
                            siz,
                            &texture as *const _ as *const std::ffi::c_void,
                            true,
                        ),
                    )?;
                }
            ...
            BufferSource::OpenGL { texture, .. } => {
                if let Some(ref tex) = self.image_src {
                    ...
                    if tex.1 != texture as u64 {
                        ...
                        _temp1 = Some(Image::<u8>::from_gl_texture(
                            self.queue.clone(),
                            MemFlags::new().read_only(),
                            desc,
                            GlTextureTarget::GlTexture2d,
            ...
            BufferSource::OpenGL { texture, .. } => {
                if let Some(ref tex) = self.image_dst {
                    ...
                    if tex.1 != *texture as u64 {
                        ...
                        _temp2 = Some(Image::<u8>::from_gl_texture(
                            self.queue.clone(),
                            MemFlags::new().write_only(),
                            desc,
                            GlTextureTarget::GlTexture2d,
                ...
                if let Some(ref tex) = self.image_dst {
                    ...
                    if let SpatialDims::Three(w, h, d) = tex.0.dims() {
                        let _ = self
                            .dst
                            .cmd()
                            .copy_to_image(&tex.0, [0, 0, 0], [*w, *h, *d])
                            .enq();
                    }
                    tex.0.cmd().d3d11_release().enq()?;
                }
