...
pub fn sample_with_background_at(
    mut uv: Vec2,
    coeffs: &[f32],
    input: &ImageType,
    params: &KernelParams,
    ...
    match params.background_mode {
        ...
        2 => {
            ...
            if rx > width3 {
                uv.x = width3 - (rx - width3);
            }
            if rx < 3.0 {
                uv.x = 3.0 + width_f - (width3 + rx);
            }
            if ry > height3 {
                uv.y = height3 - (ry - height3);
            }
            if ry < 3.0 {
                uv.y = 3.0 + height_f - (height3 + ry);
            }
            sample_input_at(uv, coeffs, input, params, sampler, interpolation, flags)
        }
        3 => {
            ...
            let mut pt2 = uv;
            let mut alpha = 1.0;
            if (uv.x > size.x - feather)
                || (uv.x < feather)
                || (uv.y > size.y - feather)
                || (uv.y < feather)
            {
                alpha = ((size.x - uv.x).min(size.y - uv.y).min(uv.x).min(uv.y) / feather)
                    .min(1.0)
                    .max(0.0);
