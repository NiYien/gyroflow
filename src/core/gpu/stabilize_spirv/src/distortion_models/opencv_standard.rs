...
impl OpenCVStandard {
    pub fn undistort_point(point: Vec2, params: &KernelParams) -> Vec2 {
        ...
        while i < 20 {
            // for _ in 0..20 {
            let r2 = x * x + y * y;
            let icdist = (1.0 + ((params.k2.w * r2 + params.k2.z) * r2 + params.k2.y) * r2)
                / (1.0 + ((params.k2.x * r2 + params.k1.y) * r2 + params.k1.x) * r2);
            if icdist < 0.0 {
                // log::warn!("icdist < 0");
                return vec2(-99999.0, -99999.0);
            }
            let delta_x = 2.0 * params.k1.z * x * y
                + params.k1.w * (r2 + 2.0 * x * x)
                + params.k3.x * r2
                + params.k3.y * r2 * r2;
            let delta_y = params.k1.z * (r2 + 2.0 * y * y)
                + 2.0 * params.k1.w * x * y
                + params.k3.z * r2
                + params.k3.w * r2 * r2;
            x = (x0 - delta_x) * icdist;
