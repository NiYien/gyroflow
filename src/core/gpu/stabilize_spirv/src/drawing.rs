...
pub fn draw_pixel(
    ...
    let data = {
        let pos_byte = fast_round(
            fast_floor(y / params.canvas_scale) as f32 * (width as f32 / params.canvas_scale)
                + fast_floor(x / params.canvas_scale) as f32,
        ) as usize;
...
pub fn draw_safe_area(in_pix: Vec4, x: f32, y: f32, params: &KernelParams) -> Vec4 {
    let mut pix = in_pix;
    let is_safe_area = x >= params.safe_area_rect.x
    ...
    if !is_safe_area {
        ...
        pix.y *= 0.5;
        pix.z *= 0.5;
        let is_border = x > params.safe_area_rect.x - 5.0
...
fn remap_colorrange(px: Vec4, is_y: bool, max_value: f32) -> Vec4 {
    if is_y {
        ((16.0 / 255.0) * max_value) + (px * ((235.0 - 16.0) / 255.0))
    } else {
        ((16.0 / 255.0) * max_value) + (px * ((240.0 - 16.0) / 255.0))
    }
}

pub fn process_final_pixel(
    mut pixel: Vec4,
    src_pos: Vec2,
    out_pos: Vec2,
    params: &KernelParams,
    ...
    if drawing_enabled {
        if src_pos.y >= params.source_rect.y as f32
            && src_pos.y < (params.source_rect.y + params.source_rect.w) as f32
        {
            if src_pos.x >= params.source_rect.x as f32
                && src_pos.x < (params.source_rect.x + params.source_rect.z) as f32
            {
                pixel = draw_pixel(
                    pixel,
                    src_pos.x,
