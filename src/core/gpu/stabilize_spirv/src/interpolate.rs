...
pub fn sample_input_at(
    ...
    #[cfg(feature = "for_qtrhi")]
    {
        ...
        if (uv.x >= 0.0 && uv.x < params.width as f32)
            && (uv.y >= 0.0 && uv.y < params.height as f32)
        {
    ...
    #[cfg(not(feature = "for_qtrhi"))]
    {
        ...
        if params.input_rotation != 0.0 {
            uv = rotate_point(
                uv,
                params.input_rotation * (core::f32::consts::PI / 180.0),
                vec2(params.width as f32 / 2.0, params.height as f32 / 2.0),
            );
        }
        ...
        if (_flags & 32) == 32 {
            ...
            uv = vec2(
                map_coord(
                    ...
                    0.0,
                    params.width as f32,
                    params.source_rect.x as f32,
                    (params.source_rect.x + params.source_rect.z) as f32,
                ),
                map_coord(
                    uv.y,
                    0.0,
                    params.height as f32,
                    params.source_rect.y as f32,
                    (params.source_rect.y + params.source_rect.w) as f32,
                ),
            );
        }
        ...
        let mut yp = 0;
        while yp < interpolation {
            //for yp in 0..params.interpolation {
            if sy + yp >= params.source_rect.y as i32
                && sy + yp < (params.source_rect.y + params.source_rect.w) as i32
            {
                let mut xsum = Vec4::splat(0.0);
                let mut xp = 0;
                while xp < interpolation {
                    // for xp in 0..params.interpolation {
                    let pixel = if sx + xp >= params.source_rect.x as i32
                        && sx + xp < (params.source_rect.x + params.source_rect.z) as i32
                    {
                        #[cfg(target_arch = "spirv")]
                        {
                            use spirv_std::image::{sample_with, ImageWithMethods};
                            to_float(input.fetch_with(
                                glam::IVec2::new((sx + xp) as i32, (sy + yp) as i32),
                                sample_with::lod(0),
                            ))
                        }
                    ...
                    if xp >= interpolation {
                        break;
                    } // Bug in Dx12 backend, doesn't work without it for some strange reason
                }
