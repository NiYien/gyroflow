...
fn get_mtrx_param(
    ...
    #[cfg(feature = "for_qtrhi")]
    {
        ...
        matrices
            .sample_with(
                *_sampler,
                vec2(idx as f32 / 11.0, row as f32 / (_size_for_rs - 1.0)),
                sample_with::lod(0.0f32),
            )
            .x
    }
}
...
pub fn rotate_and_distort(
    ...
    digital_distortion_model: u32,
    flags: u32,
) -> Vec2 {
    let size_for_rs = if (flags & 16) == 16 {
        params.width as f32
    } else {
        params.height as f32
    };
    let mut point_3d = vec3(
        (pos.x * get_mtrx_param(size_for_rs, matrices, sampler, idx, 0))
            + (pos.y * get_mtrx_param(size_for_rs, matrices, sampler, idx, 1))
            + get_mtrx_param(size_for_rs, matrices, sampler, idx, 2)
            + params.translation3d.x,
        (pos.x * get_mtrx_param(size_for_rs, matrices, sampler, idx, 3))
            + (pos.y * get_mtrx_param(size_for_rs, matrices, sampler, idx, 4))
            + get_mtrx_param(size_for_rs, matrices, sampler, idx, 5)
            + params.translation3d.y,
        (pos.x * get_mtrx_param(size_for_rs, matrices, sampler, idx, 6))
            + (pos.y * get_mtrx_param(size_for_rs, matrices, sampler, idx, 7))
            + get_mtrx_param(size_for_rs, matrices, sampler, idx, 8)
            + params.translation3d.z,
    );
    if point_3d.z > 0.0 {
        if params.r_limit > 0.0
            && vec2(point_3d.x / point_3d.z, point_3d.y / point_3d.z).length_squared()
                > params.r_limit.powi(2)
        {
...
pub fn undistort(
    ...
    let mut out_pos = if (flags & 64) == 64 {
        // Uses output rect
        vec2(
            map_coord(
                uv.x,
                params.output_rect.x as f32,
                (params.output_rect.x + params.output_rect.z) as f32,
                0.0,
                params.output_width as f32,
            ),
            map_coord(
                uv.y,
                params.output_rect.y as f32,
                (params.output_rect.y + params.output_rect.w) as f32,
                0.0,
    ...
    if out_pos.x < 0.0
        || out_pos.y < 0.0
        || out_pos.x > params.output_width as f32
        || out_pos.y > params.output_height as f32
    {
        return bg;
    }
    ...
    if params.post_zoom != 1.0
        || params.post_scale.x != 1.0
        || params.post_scale.y != 1.0
        || params.post_rotation != 0.0
        || params.post_offset.x != 0.0
        || params.post_offset.y != 0.0
    {
        ...
        out_pos = out_pos / (params.post_zoom * params.post_scale);
        out_pos = out_pos + oc;
    ...
    let mut sy = if (flags & 16) == 16 {
        // Horizontal RS
        (fast_round(out_pos.x) as f32)
            .min(params.width as f32)
            .max(0.0)
    } else {
        (fast_round(out_pos.y) as f32)
            .min(params.height as f32)
            .max(0.0)
    };
    if params.matrix_count > 1 {
        let idx = params.matrix_count / 2;
        let pt = rotate_and_distort(
            out_pos,
            idx,
            params,
            matrices,
