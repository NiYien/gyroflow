...
use super::{
    CameraStabData, FileMetadata, MESH_HEADER, MeshCorrections, MeshFrame, MeshTable, TimeIMU,
    splines,
};
use crate::stabilization::distortion_models::sony::{MAX_SEGMENTS, Sony};
use argmin::{
...
use telemetry_parser::tags_impl::{
    GetWithType, GroupId, GroupedTagMap, TagId, TagMap, TagValue, TimeVector3,
};
...
mod packet_timing_tests {
    ...
    #[test]
    fn gyro_packet_timing_respects_recorded_units_and_rejects_zero_rate() {
        use telemetry_parser::tags_impl::{TagDescription, ValueType};
        let integer = |v| TagValue::i32(ValueType::new_parsed(|x| x.to_string(), v, Vec::new()));
        let mut tags = TagMap::new();
        for (id, value) in [
            (TagId::Frequency, integer(500)),
            (TagId::Unknown(0xe436), integer(2_000_000)),
            (TagId::TimeOffset, TagValue::f64(ValueType::new_parsed(|x| x.to_string(), 4.0, Vec::new()))),
        ] {
            tags.insert(id.clone(), TagDescription { group: GroupId::Gyroscope, id, native_id: None, description: String::new(), value });
        }
        let timing = gyro_packet_timing(&tags).unwrap();
        assert_eq!(timing.offset_ms, 2.0);
        assert_eq!(timing.period_ms(), 2.0);
        tags.get_mut(&TagId::Frequency).unwrap().value = integer(0);
        assert!(gyro_packet_timing(&tags).is_none());
    }
}
...
pub fn init_lens_profile(
    ...
    if let Some(lmd) = tag_map.get(&GroupId::Custom("LensDistortion".into())) {
        let pixel_pitch = tag_map
            .get(&GroupId::Imager)
            .and_then(|x| x.get_t(TagId::PixelPitch) as Option<&(u32, u32)>)
            .cloned();
        let crop_size = tag_map
            .get(&GroupId::Imager)
            .and_then(|x| x.get_t(TagId::CaptureAreaSize) as Option<&(f32, f32)>)
            .cloned();
        let crop_origin = tag_map
            .get(&GroupId::Imager)
            .and_then(|x| x.get_t(TagId::CaptureAreaOrigin) as Option<&(f32, f32)>)
            .cloned();
        let sensor_size_px = tag_map
            .get(&GroupId::Imager)
            .and_then(|x| x.get_t(TagId::SensorSizePixels) as Option<&(u32, u32)>)
            .cloned();
        ...
        let principal_point = (|| -> Option<(f32, f32)> {
            ...
            if crop_size.0 <= 0.0 || crop_size.1 <= 0.0 {
                return None;
            }
            let sensor_size = (
                if crop_origin.0 + crop_size.0 > sensor_size.0 as f32 {
                    crop_size.0 + crop_origin.0 * 2.0
                } else {
                    sensor_size.0 as f32
                } as f64,
                if crop_origin.1 + crop_size.1 > sensor_size.1 as f32 {
                    crop_size.1 + crop_origin.1 * 2.0
                } else {
                    sensor_size.1 as f32
                } as f64,
            );
            Some((
                ((sensor_size.0 / 2.0 - crop_origin.0 as f64) * size.0 as f64 / crop_size.0 as f64)
                    as f32,
                ((sensor_size.1 / 2.0 - crop_origin.1 as f64) * size.1 as f64 / crop_size.1 as f64)
                    as f32,
            ))
        })();
        ...
        if let Some(v) = lmd.get_t(TagId::Data) as Option<&serde_json::Value> {
            telemetry_parser::try_block!({
                ...
                let approx_focal_length_mm = tag_map
                    .get(&GroupId::Lens)
                    .and_then(|x| x.get_t(TagId::FocalLength) as Option<&f32>)
                    .map(|x| *x as f64)
                ...
                let lens_in_ray_angle: Vec<f64> = v
                    .get("coeffs")?
                    .as_array()?
                    .into_iter()
                    .filter_map(|x| {
                        Some(x.as_f64()? / coeff_scale.max(1.0) / 180.0 * std::f64::consts::PI)
                    })
                    .collect();
                if lens_in_ray_angle.is_empty() || sensor_height == 0.0 || is_bad_focal_length {
                    ...
                    let focal_length_mm = if is_bad_focal_length {
                        approx_focal_length_mm
                    } else {
                        v.get("focal_length_nm")?.as_f64()? / 1000000.0
                    };
                    ...
                    let sw = tag_map
                        .get(&GroupId::Default)
                        .and_then(|x| x.get_t(TagId::SensorWidth) as Option<&f32>)
                        .map(|x| *x)
                        .unwrap_or_default() as f64
                        * sws;
                    let sh = tag_map
                        .get(&GroupId::Default)
                        .and_then(|x| x.get_t(TagId::SensorHeight) as Option<&f32>)
                        .map(|x| *x)
                        .unwrap_or_default() as f64
                        * shs;
                ...
                let pixel_pitch_m =
                    nalgebra::Vector2::new(pixel_pitch.0 as f64, pixel_pitch.1 as f64) / 1e9;
                ...
                let pixel_pitch_scaled = pixel_pitch_m
                    .component_mul(&sensor_crop_px)
                    .component_div(&video_res_px);
                ...
                if lens_in_ray_angle.len() != MAX_SEGMENTS {
                    log::warn!(
                        "Sony lens curve has {} knots, expected {MAX_SEGMENTS}",
                        lens_in_ray_angle.len()
                    );
                }
                let n = lens_in_ray_angle.len().min(MAX_SEGMENTS);
                let normalized = Sony::coefficients_from_lens_curve(
                    &lens_in_ray_angle,
                    sensor_height / n as f64 / f_meters,
                );
...
pub fn upgrade_legacy_mesh_buffers(md: &mut FileMetadata) {
    if !md.legacy_mesh_correction.is_empty() {
        md.mesh_correction =
            MeshCorrections::from_legacy(std::mem::take(&mut md.legacy_mesh_correction));
    }
}
...
pub fn upgrade_legacy_lens_params(md: &mut FileMetadata) {
    let lens_profile = match md.lens_profile.as_ref() {
        Some(x) => x,
        None => return,
    };
    if lens_profile
    ...
    let half_diagonal_px = lens_profile.get("calib_dimension").and_then(|d| {
        Some((d.get("w")?.as_f64()?.powi(2) + d.get("h")?.as_f64()?.powi(2)).sqrt() / 2.0)
    });
    for lp in md.lens_params.values_mut() {
        if lp.distortion_coefficients.len() == 6 {
            match (half_diagonal_px, lp.pixel_focal_length) {
                (Some(hd), Some((fx, _))) if fx > 0.0 => {
                    lp.distortion_coefficients = Sony::coefficients_from_legacy_polynomial(
...
pub fn retime_imu_from_packets(
    imu: &mut Vec<TimeIMU>,
    ...
    for info in samples {
        let Some(gyro) = info
            .tag_map
            .as_ref()
            .and_then(|tm| tm.get(&GroupId::Gyroscope))
        else {
            continue;
        };
        ...
        let Some(timing) = gyro_packet_timing(gyro) else {
            return false;
        };
        packets.push(Packet {
            start_ms: info.timestamp_ms + timing.offset_ms,
    ...
    if total == 0 || total != imu.len() {
        log::warn!(
            "Sony gyro packets ({total}) don't match the IMU sample count ({}), keeping the uniform timing",
            imu.len()
        );
        return false;
    }
    ...
    log::debug!(
        "Sony gyro samples re-timed from {} packets in {:?}{}",
        packets.len(),
        started.elapsed(),
        if monotonic { "" } else { " (sorted)" }
    );
    true
}
...
pub fn get_time_offset(
    ...
    if packet_timed {
        ...
        let centre = (|| -> Option<f64> {
            ...
            let sensor_h = if origin.1 + size.1 > sensor.1 as f32 {
                size.1 + origin.1 * 2.0
            } else {
                sensor.1 as f32
            } as f64;
        ...
        .unwrap_or(0.5);
        let frame_offset =
            first_frame_ts - (exposure_time / 2.0) + readout_time * centre + model_offset;
    ...
    let offset_diff =
        (timing.offset_us - period_us * (timing.offset_us / period_us).floor()).round() / 1000.0;

    let frame_offset =
        first_frame_ts - (exposure_time / 2.0) + (readout_time / 2.0) + model_offset + offset_diff
            - timing.offset_ms;

    Some((
        original_sample_rate,
        frame_offset / timing.frequency * sample_rate,
    ))
}
...
impl ISSamples {
    fn calc_time_diff(&self, frame_interval: i32, i1: usize, i2: usize) -> Option<i32> {
        ...
        if dt <= 0 {
            // wrapped to the next frame, or a duplicated/clamped sample
            dt += frame_interval;
        }
        Some(dt)
    }
    ...
    fn search_top_idx2(
        ...
        let (mut top_index, mut current_time) =
            self.search_idx(frame_interval, frame, top_offset, top_offset)?;
        let adj = if top_offset >= 0.0 { 2 } else { 1 };
        for _i in 0..adj {
            if top_index > 0 {
                current_time -=
                    self.calc_time_diff(frame_interval, top_index - 1, top_index)? as f64;
                top_index -= 1;
            }
        }
    ...
    fn search_bot_idx2(
        ...
        let (mut bot_index, mut current_time) =
            self.search_idx(frame_interval, frame, top_offset, bot_offset)?;
        let adj = if bot_offset >= 0.0 { 2 } else { 1 };
        for _i in 0..adj {
            if bot_index > 0 {
                current_time +=
                    self.calc_time_diff(frame_interval, bot_index, bot_index + 1)? as f64;
                bot_index += 1;
            }
        }
    ...
    fn calc_spline(
        ...
        readout_time: f64,
        entry_rate: f64,
    ) -> Option<(splines::CatmullRom<nalgebra::Vector3<f64>>, f64)> {
        let mut spline = splines::CatmullRom::new();
        if self.t.is_empty() {
            return Some((spline, 0.0));
        }
        let bot_offset = top_offset + readout_time;
        let (top_index, time) = self.search_top_idx2(frame_interval, frame, top_offset)?;
        let (bot_index, bot_time) =
            self.search_bot_idx2(frame_interval, frame, top_offset, bot_offset)?;
        ...
        let ofs_rows = if bot_time - time >= readout_time {
            ((time - top_offset).abs() * entry_rate) as i64
        } else {
            0
        };
...
pub fn stab_collect(
    is: &mut ISTemp,
    tag_map: &GroupedTagMap,
    _info: &telemetry_parser::util::SampleInfo,
    frame_rate: f64,
) -> Option<()> {
    let imager = tag_map.get(&GroupId::Imager)?;
    ...
    if let Some(shift) = ibis.and_then(|x| x.get_t(TagId::Data) as Option<&Vec<TimeVector3<i32>>>) {
        ...
        if angle.map_or(true, |a| a.len() != shift.len()) {
            log::warn!(
                "IBIS position and rotation sample counts differ: {} vs {:?}",
                shift.len(),
                angle.map(|a| a.len())
            );
        }
        for (i, s) in shift.iter().enumerate() {
            ...
            is.ibis
                .a
                .push(angle.and_then(|a| a.get(i)).map(|a| a.z).unwrap_or(0));
        }
    }
    if let Some(ois) = ois {
    ...
    is.per_frame_crop
        .push((crop_origin.0, crop_origin.1, crop_size.0, crop_size.1));
...
pub fn stab_calc_splines(
    ...
    let per_frame_data: Vec<CameraStabData> = (0..num_frames)
        ...
        .filter_map(|frame| {
            ...
            let (ibis_spline, offset) = is_temp
                .ibis
                .calc_spline(
                    is_temp.frame_interval,
                    frame,
                    top_offset,
                    readout_time,
                    entry_rate,
                )
                .unwrap_or_else(|| (splines::CatmullRom::new(), 0.0));
            let (ois_spline, ois_offset) = is_temp
                .ois
                .calc_spline(
                    is_temp.frame_interval,
                    frame,
    ...
    if per_frame_data.len() != num_frames {
        log::error!(
            "Sony stabilizer data is incomplete: {} of {num_frames} frames",
            per_frame_data.len()
        );
        return None;
    }
...
pub fn get_mesh_correction(
    tag_map: &GroupedTagMap,
    video_size: (usize, usize),
    meshes: &mut MeshCorrections,
    ...
    let focal_plane_data =
        focal_plane_group.and_then(|x| x.get_t(TagId::Data) as Option<&serde_json::Value>);
    ...
    let focal_plane = focal_plane_data
        .and_then(parse_focal_plane)
        .unwrap_or_default();
    ...
    let divisions = (|| -> Option<(usize, usize)> {
        ...
        Some((
            divisions[0].as_i64()? as usize,
            divisions[1].as_i64()? as usize,
        ))
    })()
    .unwrap_or((0, 0));
...
fn parse_focal_plane(data: &serde_json::Value) -> Option<Vec<f64>> {
    ...
    let mut coords = vec![
        bands.len() as f64,
        data.get("unk1")?.as_i64()? as f64,
        data.get("unk2")?.as_i64()? as f64,
        data.get("scale")?.as_f64()?,
    ];
    for x in bands {
        let coord = x.as_array()?;
...
fn mesh_key(
    ...
    feed(size.0);
    feed(size.1);
    feed(divisions.0 as f64);
    feed(divisions.1 as f64);
    feed(crop_size.0 as f64);
    feed(crop_size.1 as f64);
...
pub fn inverse_residual(forward: &[f64], inverse: &[f64], size: (f64, f64)) -> f64 {
    ...
    (0..n * n)
        ...
        .map(|i| {
            let q = (
                size.0 * (i % n) as f64 / (n - 1) as f64,
                size.1 * (i / n) as f64 / (n - 1) as f64,
            );
...
fn build_mesh_table(
    ...
    if divisions.0 < 2
        || divisions.1 < 2
        || divisions.0 > splines::MAX_GRID_SIZE
        || divisions.1 > splines::MAX_GRID_SIZE
        || !(size.0 > 0.0)
        || !(size.1 > 0.0)
    {
    ...
    if nodes.len() != divisions.0 * divisions.1 {
        log::error!(
            "Invalid mesh correction: {} nodes for {divisions:?} divisions",
            nodes.len()
        );
        return None;
    }
    let capacity =
        MESH_HEADER + divisions.0 * divisions.1 * 2 + divisions.1 * splines::MAX_GRID_SIZE * 4 * 2;
    ...
    let step = (
        size.0 / (divisions.0 - 1) as f64,
        size.1 / (divisions.1 - 1) as f64,
    );
    let grid: Vec<(f64, f64)> = (0..divisions.1)
        .flat_map(|y| (0..divisions.0).map(move |x| (step.0 * x as f64, step.1 * y as f64)))
        .collect();
    let inverted: Option<Vec<(f64, f64)>> = grid
        .into_par_iter()
        .map(|(x, y)| inverse_interpolate_mesh(x, y, size, &mesh).ok())
        .collect();
    ...
    let video_px_per_sensor_px =
        if video_size.0 > 0 && video_size.1 > 0 && crop_size.0 > 0.0 && crop_size.1 > 0.0 {
    ...
    Some(MeshTable {
        refinement: if refine {
            mesh.iter().map(|x| *x as f32).collect()
        } else {
            Vec::new()
        },
...
fn inverse_interpolate_mesh(
    x_prime: f64,
    y_prime: f64,
    size: (f64, f64),
    mesh: &[f64],
) -> Result<(f64, f64), argmin::core::Error> {
    let operator = Objective {
        x_prime,
        y_prime,
        size,
        mesh,
    ...
    if let Some(coeffs) = res.state.best_param {
        ...
        Err(argmin::core::Error::new(
            argmin::core::ArgminError::InvalidParameter {
                text: String::new(),
            },
        ))
    }
