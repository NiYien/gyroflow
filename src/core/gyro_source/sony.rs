...
use telemetry_parser::tags_impl::{GetWithType, GroupId, GroupedTagMap, TagId, TimeVector3};
    ...
    if let Some(lmd) = tag_map.get(&GroupId::Custom("LensDistortion".into())) {
        ...
        if let Some(v) = lmd.get_t(TagId::Data) as Option<&serde_json::Value> {
            telemetry_parser::try_block!({
                ...
                let mut lens_in_ray_angle: Vec<f64> = v
                    .get("coeffs")?
                    .as_array()?
                    .into_iter()
                    .filter_map(|x| {
                ...
                let lens_out_radius = nalgebra::DVector::from_iterator(
                    11,
                    (0..11).map(|i| (i as f64) / 10.0 * sensor_height),
                );
                ...
                match nalgebra::SVD::new(matrix.clone(), true, true)
                    .solve(&lens_out_radius, 1e-18f64)
                {
                    Ok(poly_coeffs) => {
                        ...
                        let sensor_crop_px =
                            nalgebra::Vector2::new(crop_size.0 as f64, crop_size.1 as f64);
                        let pixel_pitch =
                            nalgebra::Vector2::new(pixel_pitch.0 as f64, pixel_pitch.1 as f64)
                                / 1e9;
                        ...
                        let timestamp_us = (info.timestamp_ms * 1000.0).round() as i64;
                        if let Some(lp) = md.lens_params.get_mut(&timestamp_us) {
                            lp.focal_length = Some(
                                (focal_length * sensor_height / size.1 as f64 * 1000.0) as f32,
                            );
                            lp.pixel_focal_length = Some(focal_length as f32);
                            lp.distortion_coefficients =
                                poly_coeffs.into_iter().cloned().chain(post_scale).collect();
                        }
...
pub fn get_time_offset(
    ...
    let offset_diff = (rounded_offset
        - (1000000.0 / sampling_frequency)
            * (rounded_offset / (1000000.0 / sampling_frequency)).floor())
    .round()
        / 1000.0;

    let frame_offset = first_frame_ts - (exposure_time / 2.0)
        + (md.frame_readout_time.unwrap_or_default() / 2.0)
        + model_offset
        + offset_diff
        - offset;
    ...
    Some((
        ...
        frame_offset / sampling_frequency * sample_rate,
    ))
}
    ...
    if let Some(ois) = ois {
        if let Some(shift) = ois.get_t(TagId::Data) as Option<&Vec<TimeVector3<i32>>> {
            for s in shift.into_iter() {
                if is.ibis_x.is_empty() {
                    // if `t` was not pushed by IBIS, this means we only have OIS, so push to `t` here
                    is.t.push(s.t);
                }
                is.ois_x.push(s.x);
                is.ois_y.push(s.y);
...
pub fn stab_calc_splines(
    ...
    let per_frame_data: Vec<CameraStabData> = (0..num_frames)
        ...
        .filter_map(|frame| {
            ...
            let n_entries =
                is_temp.search_bot_idx2(frame, top_offset, bot_offset)?.0 - top_index + 1;
...
pub fn get_mesh_correction(
    tag_map: &GroupedTagMap,
    cache: &mut BTreeMap<u32, (Vec<f64>, Vec<f32>)>,
    ...
    let crc = crc32fast::hash(
        serde_json::to_string(&[
            mesh_data.unwrap_or(&serde_json::Value::Null),
            focal_plane_data.unwrap_or(&serde_json::Value::Null),
            &crop_origin.0.into(),
            &crop_origin.1.into(),
            &crop_size.0.into(),
            &crop_size.1.into(),
    ...
    let focal_plane_data = if let Some(focal_plane_data) = focal_plane_data {
        ...
        let mut coords = vec![
            focal_plane_data.get("unk4")?.as_array()?.len() as f64,
            unk1,
            unk2,
            scale,
        ];
    ...
    let mut mesh =
        Vec::with_capacity(9 + divisions.0 * divisions.1 * 2 + (divisions.1 * divisions.0 * 4 * 2));
    ...
    if has_any_mesh_value {
        let step = (
            (size.0 / (divisions.0 as f64 - 1.0)),
            (size.1 / (divisions.1 as f64 - 1.0)),
        );
        let grid: Vec<_> = (0..divisions.1)
            .map(|y| (0..divisions.0).map(move |x| (x as f64, y as f64)))
            .flatten()
            .collect();

        let new_mesh: Vec<f64> = grid
            .into_par_iter()
            .filter_map(|(x, y)| {
