...
impl BivariateSpline {
    ...
    pub fn cubic_spline_coefficients(
        mesh: &[f64],
        step: usize,
        offset: usize,
        size: f64,
    ...
    pub fn interpolate(
        ...
        let i = (self.grid_size.0 - 2)
            .min(((self.grid_size.0 as f64 - 1.0) * x / size_x) as usize)
            .max(0);
...
impl NaturalSpline {
    ...
    pub fn new(xs: &[f64], ys: &[f64]) -> Option<Self> {
        let n = xs.len();
        if n < 2 || ys.len() != n || xs.windows(2).any(|w| !(w[1] > w[0])) {
            return None;
        }
    ...
    pub fn at(&self, t: f64) -> f64 {
        ...
        if t >= self.xs[n - 1] {
            ...
            return self.a[n - 1]
                + cubic_slope(self.b[n - 2], self.c[n - 2], self.d[n - 2], h)
                    * (t - self.xs[n - 1]);
        }
        let i = self
            .xs
