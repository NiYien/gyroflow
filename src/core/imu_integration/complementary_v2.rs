...
impl ComplementaryFilterV2 {
    ...
    pub fn update(&mut self, ax: f64, ay: f64, az: f64, wx: f64, wy: f64, wz: f64, dt: f64) {
        ...
        self.time_steady = if self.steady_state {
            self.time_steady + dt
        } else {
            0.0
        };
        ...
        self.q = quaternion_multiplication(
            pred.0, pred.1, pred.2, pred.3, dq_acc.0, dq_acc.1, dq_acc.2, dq_acc.3,
        );
    ...
    pub fn update_mag(
        ...
        let q_temp = quaternion_multiplication(
            pred.0, pred.1, pred.2, pred.3, dq_acc.0, dq_acc.1, dq_acc.2, dq_acc.3,
        );
        ...
        // where qI = identity quaternion
        let mut dq_mag =
            self.get_mag_correction(mx, my, mz, q_temp.0, q_temp.1, q_temp.2, q_temp.3);
        ...
        self.q = quaternion_multiplication(
            q_temp.0, q_temp.1, q_temp.2, q_temp.3, dq_mag.0, dq_mag.1, dq_mag.2, dq_mag.3,
        );
    ...
    fn autoscale_gravity(&mut self, _ax: f64, _ay: f64, _az: f64) {
        ...
        if self.partial_steady_state {
            ...
            let acc_magnitude = (self.a_filt.0 * self.a_filt.0
                + self.a_filt.1 * self.a_filt.1
                + self.a_filt.2 * self.a_filt.2)
                .sqrt();
            // Within reasonable amount of true acceleration
            if (acc_magnitude - GRAVITY).abs() < GRAV_AUTOSCALE_THRESHOLD {
                self.gravity = self.gravity * (1.0 - GRAV_AUTOSCALE_ALPHA)
                    + GRAV_AUTOSCALE_ALPHA * acc_magnitude;
            }
        }
    }
    ...
    fn check_state(&mut self, ax: f64, ay: f64, az: f64, wx: f64, wy: f64, wz: f64) -> bool {
        ...
        self.partial_steady_state =
            acc_component_steady && acc_delta_th && gyro_delta_th && gyro_th;
    ...
    fn get_prediction(&self, wx: f64, wy: f64, wz: f64, dt: f64) -> (f64, f64, f64, f64) {
        ...
        let mut q0_pred =
            self.q.0 + 0.5 * dt * (wx_unb * self.q.1 + wy_unb * self.q.2 + wz_unb * self.q.3);
        let mut q1_pred =
            self.q.1 + 0.5 * dt * (-wx_unb * self.q.0 - wy_unb * self.q.3 + wz_unb * self.q.2);
        let mut q2_pred =
            self.q.2 + 0.5 * dt * (wx_unb * self.q.3 - wy_unb * self.q.0 - wz_unb * self.q.1);
        let mut q3_pred =
            self.q.3 + 0.5 * dt * (-wx_unb * self.q.2 + wy_unb * self.q.1 - wz_unb * self.q.0);
    ...
    fn get_measurement(&mut self, mut ax: f64, mut ay: f64, mut az: f64) -> (f64, f64, f64, f64) {
        ...
        if az >= 0.0 {
            ...
            (q0_meas, -ay / (2.0 * q0_meas), ax / (2.0 * q0_meas), 0.0)
        } else {
            let x = ((1.0 - az) * 0.5).sqrt();
            (-ay / (2.0 * x), x, 0.0, ax / (2.0 * x))
        }
    }

    fn get_measurement_mag(
        &mut self,
        mut ax: f64,
        mut ay: f64,
        mut az: f64,
        ...
        let q_acc = if az >= 0.0 {
            ...
            (q0_acc, -ay / (2.0 * q0_acc), ax / (2.0 * q0_acc), 0.0)
        } else {
            let x = ((1.0 - az) * 0.5).sqrt();
            (-ay / (2.0 * x), x, 0.0, ax / (2.0 * x))
        };
        ...
        // l = R(q_acc)^-1 m
        let lx = (q_acc.0 * q_acc.0 + q_acc.1 * q_acc.1 - q_acc.2 * q_acc.2) * mx
            + 2.0 * (q_acc.1 * q_acc.2) * my
            - 2.0 * (q_acc.0 * q_acc.2) * mz;
        let ly = 2.0 * (q_acc.1 * q_acc.2) * mx
        ...
        // q = q_acc times q_mag
        quaternion_multiplication(q_acc.0, q_acc.1, q_acc.2, q_acc.3, q0_mag, 0.0, 0.0, q3_mag)
        // (
        //    q_acc.0 * q0_mag,
    ...
    fn get_acc_correction(
        ...
        (dq0, -g.1 / (2.0 * dq0), g.0 / (2.0 * dq0), 0.0)
    }

    fn get_mag_correction(
        &mut self,
        mx: f64,
        my: f64,
        mz: f64,
    ...
    fn get_adaptive_gain(&mut self, alpha: f64, ax: f64, ay: f64, az: f64, dt: f64) -> f64 {
        if self.do_adaptive_gain {
            ...
            let w_mag = (self.w_prev.0 * self.w_prev.0
                + self.w_prev.1 * self.w_prev.1
                + self.w_prev.2 * self.w_prev.2)
                .sqrt();
            ...
            let new_gain = if self.time_steady > STEADY_WAIT_THRESHOLD {
                ...
                (-40.0 * error - 1.0 * w_mag).exp()
                    * alpha
                    * (if self.time < self.initial_settle_time {
                        (15.0 - self.time / self.initial_settle_time * 14.0).max(8.0)
                    } else {
                        1.0
                    })
            };
            // 1st order filter of gain when increasing
            let gain = if new_gain < self.prev_gain_acc {
                new_gain
            } else {
                gain_iir_alpha * new_gain + (1.0 - gain_iir_alpha) * self.prev_gain_acc
            };
