...
use super::gyro_source::{Quat64, TimeIMU, TimeQuat};
...
impl QuaternionConverter {
    pub fn convert(
        ...
        for (&org_ts, &org_quat) in org_quaternions {
            let n_quat = integrated_quats
                .range(org_ts..)
                .next()
                .map(|x| *x.1)
                .unwrap_or(UnitQuaternion::identity());
            let io_quat = image_orientations
                .range(org_ts..)
                .next()
                .map(|x| *x.1)
                .unwrap_or(UnitQuaternion::identity());
            let corr = n_quat * (org_quat * io_quat.inverse()).inverse();
            corr_sm = corr_sm.slerp(
                &corr,
                if boost > 0 {
                    boost -= 1;
                    1.0
    ...
    fn integrate(imu_data: &[TimeIMU], duration_ms: f64) -> TimeQuat {
        ...
        for v in imu_data {
            if let Some(g) = v.gyro.as_ref() {
                ...
                if a[0].abs() == 0.0 && a[1].abs() == 0.0 && a[2].abs() == 0.0 {
                    a[0] += 0.0000001;
                }
                ...
                quats.insert(
                    (v.timestamp_ms * 1000.0) as i64,
                    Quat64::from_quaternion(Quaternion::from_parts(
                        x.0,
                        Vector3::new(x.1, x.2, x.3),
                    )),
                );
    ...
    fn integrate(imu_data: &[TimeIMU], duration_ms: f64) -> TimeQuat {
        ...
        for (i, v) in imu_data.iter().enumerate() {
            out_quats.insert(
                ...
                Quat64::from_quaternion(Quaternion::from_parts(
                    quat[i * 4],
                    Vector3::new(quat[i * 4 + 1], quat[i * 4 + 2], quat[i * 4 + 3]),
                )),
            );
        }
        out_quats
    ...
    fn integrate(imu_data: &[TimeIMU], duration_ms: f64) -> TimeQuat {
        ...
        for v in imu_data {
            if let Some(g) = v.gyro.as_ref() {
                ...
                orientation =
                    Quat64::from_quaternion(orientation.quaternion() * delta_q.quaternion());
        ...
        for v in imu_data {
            if let Some(g) = v.gyro.as_ref() {
                ...
                if (0.9..1.1).contains(&g) {
                    ...
                    let weight = if v.timestamp_ms - start_time < 15000.0 {
                        10.0
                    } else {
                        0.6
                    };
                ...
                *ahrs.sample_period_mut() = (v.timestamp_ms - prev_time) / 1000.0;
                ...
                if let Some(m) = v.magn.as_ref() {
                    ...
                    match ahrs.update(&gyro, &accl, &magn) {
                        Ok(quat) => {
                            quats.insert((v.timestamp_ms * 1000.0) as i64, *quat);
                        }
                        Err(e) => log::warn!(
                            "Invalid data! {:?} Gyro: [{}, {}, {}] Accel: [{}, {}, {}] Magn: [{}, {}, {}]",
                            e,
                            gyro[0],
                            gyro[1],
                            gyro[2],
