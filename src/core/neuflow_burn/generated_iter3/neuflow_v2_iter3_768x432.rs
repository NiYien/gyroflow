// Generated from ONNX "C:/Users/Jhe/Desktop/github/gyroflow-burn-neuflow/neuflow_v2_iter3_768x432.onnx" by burn-onnx
use burn::prelude::*;
...
impl<B: Backend> Submodule3<B> {
    ...
    pub fn new(device: &B::Device) -> Self {
        ...
        let conv2d40 = Conv2dConfig::new([96, 66], [3, 3])
            ...
            .with_bias(true)
            .init(device);
        let conv2d41 = Conv2dConfig::new([3, 128], [8, 8])
            .with_stride([8, 8])
            .with_padding(PaddingConfig2d::Valid)
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d42 = Conv2dConfig::new([128, 128], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1, 1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d43 = Conv2dConfig::new([130, 256], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1, 1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d44 = Conv2dConfig::new([256, 512], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1, 1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d45 = Conv2dConfig::new([512, 576], [1, 1])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Valid)
            .with_dilation([1, 1])
            .with_groups(1)
    ...
    pub fn forward(
        ...
        let slice14_out1 = conv2d40_out1.slice(s![.., 0..2, .., ..]);
        let add15_out1 = add12_out1.add(slice14_out1);
        let conv2d41_out1 = self.conv2d41.forward(div1_out1);
        ...
        let concat17_out1 = burn::tensor::Tensor::cat(
            [add15_out1.clone(), leakyrelu35_out1].into(),
            1,
        );
        let conv2d43_out1 = self.conv2d43.forward(concat17_out1);
        let relu3_out1 = burn::tensor::activation::relu(conv2d43_out1);
        let conv2d44_out1 = self.conv2d44.forward(relu3_out1);
        let relu4_out1 = burn::tensor::activation::relu(conv2d44_out1);
        let conv2d45_out1 = self.conv2d45.forward(relu4_out1);
        let reshape18_out1 = conv2d45_out1.reshape([1, 1, 9, 8, 8, 54, 96]);
        let softmax4_out1 = {
            let dtype = reshape18_out1.dtype();
            burn::tensor::activation::softmax(reshape18_out1.cast(burn::tensor::DType::F32), 2)
                .cast(dtype)
        };
        let pad1_out1 = add15_out1
            .pad(
        ...
        let transpose15_out1 = gather2_out1.permute([0, 1, 2, 4, 3, 5]);
        let reshape19_out1 = transpose15_out1.reshape([1, 2, 9, 1, 1, 54, 96]);
        let mul22_out1 = softmax4_out1.mul(reshape19_out1);
        let reducesum1_out1 = {
            let dtype = mul22_out1.dtype();
            mul22_out1.cast(burn::tensor::DType::F32)
                .sum_dim(2usize).squeeze_dims::<6usize>(&[2])
                .cast(dtype)
        };
        let transpose16_out1 = reducesum1_out1.permute([0, 1, 4, 2, 5, 3]);
        let reshape20_out1 = transpose16_out1.reshape([1, 2, 432, 768]);
        let constant104_out1 = self.constant104.val();
        let mul23_out1 = reshape20_out1
...
impl<B: Backend> Default for Model<B> {
    fn default() -> Self {
        Self::from_file(
            "C:/Users/Jhe/Desktop/github/gyroflow-burn-neuflow/src/core/neuflow_burn/generated_iter3/neuflow_v2_iter3_768x432.bpk",
            &Default::default(),
        )
    }
}
