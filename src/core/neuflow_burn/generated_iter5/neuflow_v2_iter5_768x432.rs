...
impl<B: Backend> Submodule4<B> {
    ...
    pub fn new(device: &B::Device) -> Self {
        ...
        let conv2d56 = Conv2dConfig::new([96, 66], [3, 3])
            ...
            .with_bias(true)
            .init(device);
        let conv2d57 = Conv2dConfig::new([3, 128], [8, 8])
            .with_stride([8, 8])
            .with_padding(PaddingConfig2d::Valid)
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d58 = Conv2dConfig::new([128, 128], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1, 1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d59 = Conv2dConfig::new([130, 256], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1, 1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d60 = Conv2dConfig::new([256, 512], [3, 3])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Explicit(1, 1, 1, 1))
            .with_dilation([1, 1])
            .with_groups(1)
            .with_bias(true)
            .init(device);
        let conv2d61 = Conv2dConfig::new([512, 576], [1, 1])
            .with_stride([1, 1])
            .with_padding(PaddingConfig2d::Valid)
            .with_dilation([1, 1])
            .with_groups(1)
    ...
    pub fn forward(
        ...
        let relu4_out1 = burn::tensor::activation::relu(conv2d60_out1);
        let conv2d61_out1 = self.conv2d61.forward(relu4_out1);
        let reshape22_out1 = conv2d61_out1.reshape([1, 1, 9, 8, 8, 54, 96]);
        let softmax4_out1 = {
            let dtype = reshape22_out1.dtype();
        ...
        let transpose19_out1 = gather2_out1.permute([0, 1, 2, 4, 3, 5]);
        let reshape23_out1 = transpose19_out1.reshape([1, 2, 9, 1, 1, 54, 96]);
...
impl<B: Backend> Default for Model<B> {
    fn default() -> Self {
        Self::from_file(
            "C:/Users/Jhe/Desktop/github/gyroflow-burn-neuflow/src/core/neuflow_burn/generated_iter5/neuflow_v2_iter5_768x432.bpk",
            &Default::default(),
        )
    }
}
