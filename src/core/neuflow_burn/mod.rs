...
// Generated by build.rs via burn-onnx ModelGen. The .rs/.bpk land in
// $OUT_DIR/burn_onnx/. burn 0.22 drops the Backend `B` generic on Tensor and
// uses a unified Device type — the generated code reflects that.
#[allow(
...
mod neuflow_v2_generated_fp16 {
    include!(concat!(env!("OUT_DIR"), "/burn_onnx/neuflow_v2_iter5_768x432.rs"));
}
...
///
/// Must be called before any cubecl `CubeClRuntimeConfig::get()` or Wgpu device init,
/// otherwise `CubeClRuntimeConfig::set` panics. Call at the very top of main before
/// wgpu GPU probing (see `util::init_logging` in `gyroflow.rs`).
pub fn init_cubecl_cache() {
    use cubecl_runtime::config::{
        CubeClRuntimeConfig, autotune::AutotuneConfig, cache::CacheConfig,
...
fn cubecl_autotune_level() -> cubecl_runtime::config::autotune::AutotuneLevel {
    use cubecl_runtime::config::autotune::AutotuneLevel;

    match std::env::var("CUBECL_AUTOTUNE_LEVEL").ok().as_deref() {
        Some("balanced" | "1") => AutotuneLevel::Balanced,
        Some("extensive" | "2") => AutotuneLevel::Extensive,
        Some("full" | "3") => AutotuneLevel::Full,
        Some("minimal" | "0") => AutotuneLevel::Minimal,
...
/// Create GPU tensors from owned f32 data. burn 0.22 needs an explicit
/// `(device, DType)` second arg so we land the tensor directly in F16 to match
/// the model's F16 weights — without this the burn-ir builder hits DTypeMismatch.
fn prepare_tensors(
    img0: Vec<f32>,
    img1: Vec<f32>,
    h: usize,
    ...
    let img0_tensor =
        Tensor::<4>::from_data(img0_data, (device, burn::tensor::DType::F16));
    let img1_tensor =
        Tensor::<4>::from_data(img1_data, (device, burn::tensor::DType::F16));
...
fn readback_flow_sync(flow_tensor: Tensor<4>, h: usize, w: usize) -> Result<Vec<f32>, String> {
    ...
    loop {
        ...
        match request {
            InferRequest::Infer {
                ...
                let mut t_tensor = t0;
                let mut t_fwd = t0;
                let mut flow_tensor: Option<Tensor<4>> = None;
            ...
            InferRequest::InferAndSample {
                ...
                if let Some(prev) = deferred.take() {
                    ...
                    let do_forward_select = || {
                        ...
                        drop(_trace_guard);
                        let plane = h * w;
                        let idx_t = Tensor::<1, burn::tensor::Int>::from_data(
...
fn run_warmup(model: &Model, device: &Device) -> Result<(), String> {
    let h = 432usize;
    let w = 768usize;
...
fn init_model() -> Result<(Model, Device), String> {
    let path =
