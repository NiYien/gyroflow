...
pub fn extract_effective_focal_lengths(params: &ComputeParams) -> Vec<Option<f64>> {
    ...
    (0..params.frame_count)
        .map(|frame| {
            let lens_timestamp_us = ComputeParams::lens_timestamp_us_with_delay(
                crate::timestamp_at_frame(frame as i32, params.scaled_fps),
                0,
                params.scaled_fps,
            );
            // On the metadata borrowed above: a second read of a lock this thread already holds deadlocks once a writer queues up
            let (camera_matrix, _, _, _, _, _, per_frame) =
                FrameTransform::get_lens_data_at_lens_timestamp(
                    params,
                    &file_metadata,
                    lens_timestamp_us,
                    false,
...
pub fn extract_lens_zoom(params: &ComputeParams) -> Vec<Option<(f64, f64)>> {
    ...
    (0..params.frame_count)
        .map(|frame| {
            let timestamp_us = ComputeParams::lens_timestamp_us_with_delay(
                crate::timestamp_at_frame(frame as i32, params.scaled_fps),
                0,
                params.scaled_fps,
            );
            let lp = file_metadata
                .lens_params_closest(timestamp_us, 100000, |v| v.has_projection_data())?;
...
pub fn ring_hint(raw_px: &[f64], lens_zoom: &[Option<(f64, f64)>]) -> Option<Vec<Option<f64>>> {
    let samples: Vec<(f64, f64)> = lens_zoom
        .iter()
        .filter_map(|s| {
            s.and_then(|(mm, ring)| {
                (mm > 0.0 && ring > 0.01 && ring < 99.99).then(|| (ring, mm.ln()))
            })
        })
        .collect();
    if samples.len() < RING_MIN_FRAMES {
        return None;
    }
    let (lo, hi) = samples.iter().fold((f64::MAX, f64::MIN), |(lo, hi), s| {
        (lo.min(s.0), hi.max(s.0))
    });
    if hi - lo < RING_MIN_TRAVEL {
        return None;
    }
    ...
    let monotonic = |c: &[f64]| {
        (0..200).all(|i| {
            polyval(c, -1.0 + i as f64 / 100.0) < polyval(c, -1.0 + (i + 1) as f64 / 100.0)
        })
    };
    let coeffs = [3usize, 1]
        .iter()
        .find_map(|&deg| polyfit(&xs, &ys, deg).filter(|c| monotonic(c)))?;
    ...
    let expected = if step > 0.0 {
        mms.iter().map(|mm| step / mm).sum::<f64>() / mms.len() as f64 / 12f64.sqrt()
    } else {
        0.0
    };
    let residual = (xs
        ...
        / xs.len() as f64)
        .sqrt();
    if residual > RING_MAX_RESIDUAL_FACTOR * expected + 0.002 {
        return None;
    }
    ...
    if ring_changes < 4
        || (step > 0.0 && ring_quantum_ln > RING_MAX_STEP_FRACTION * expected * 12f64.sqrt())
    {
        return None;
    }
...
pub fn remove_outliers(v: &mut [f64], max_frames: usize) -> usize {
    ...
    while i < n {
        ...
        if base > 0.0 && v[i] > 0.0 && (v[i] / base).ln().abs() > OUTLIER_LN_RATIO {
            let back = (i + 1..=(i + max_frames).min(n - 1))
                .find(|&j| v[j] > 0.0 && (v[j] / base).ln().abs() <= OUTLIER_LN_RETURN);
            if let Some(j) = back {
                for k in i..j {
...
fn fill_gaps(v: &mut [Option<f64>]) -> bool {
    let Some(first) = v.iter().position(|x| x.is_some()) else {
        return false;
    };
...
pub fn dequantize(v: &[f64], hint: Option<&[Option<f64>]>) -> Vec<f64> {
    ...
    let is_plateau = |run: &(usize, usize, f64), neighbour: &(usize, usize, f64)| {
        run.1 > DEQUANTIZE_PLATEAU_RATIO * neighbour.1 && run.1 > typical
    };
    let q = quantization_step(v);
    let is_jump = |a: f64, b: f64| {
        let d = (b - a).abs();
        d > DEQUANTIZE_JUMP_RATIO * a.min(b)
            || (q > 0.0 && d > DEQUANTIZE_JUMP_STEPS * q && d > DEQUANTIZE_MAX_STEP * a.min(b))
    };
    ...
    for pair in runs.windows(2) {
        ...
        let ka = if is_plateau(a, b) {
            ((a.0 + a.1) as f64 - 0.5 - b.1 as f64 / 2.0).max(midpoint(a))
        } else {
            midpoint(a)
        };
        let kb = if is_plateau(b, a) {
            (b.0 as f64 - 0.5 + a.1 as f64 / 2.0).min(midpoint(b))
        } else {
            midpoint(b)
        };
        if kb <= ka {
    ...
    let smooth_segments = |out: &mut Vec<f64>, frames: usize| {
        ...
        for pair in segment_starts.windows(2) {
            ...
            for (i, s) in smoothed.into_iter().enumerate() {
                out[start + i] = match band {
                    Some(half) => s.clamp(v[start + i] - half, v[start + i] + half),
                    None => s,
                };
            }
        }
    ...
    if let Some(hint) = hint {
        ...
        while i < hint.len().min(out.len()) {
            ...
            let start = i;
            while i < hint.len().min(out.len()) && hint[i].is_some() {
                i += 1;
            }
            let end = i;
            for j in start..end {
                let h = hint[j].unwrap();
                let w_start = if start == 0 {
                    1.0
                } else {
                    ((j - start + 1) as f64 / (DEQUANTIZE_HINT_FADE + 1) as f64).min(1.0)
                };
                let w_end = if end == hint.len() {
                    1.0
                } else {
                    ((end - j) as f64 / (DEQUANTIZE_HINT_FADE + 1) as f64).min(1.0)
                };
                let w = w_start.min(w_end);
                let blended = out[j] * (1.0 - w) + h * w;
                out[j] = match band {
                    Some(half) => blended.clamp(v[j] - half, v[j] + half),
                    None => blended,
                };
            }
        }
...
pub fn upper_envelope(
    ...
    if !trim_ranges.is_empty() {
        ...
        if let Some(least_restrictive) = s
            .iter()
            .enumerate()
            .filter(|(i, _)| within(*i))
            .map(|(_, v)| *v)
...
pub fn round_corners(s: &mut [f64], floor: &[f64], max_step: f64, max_accel: f64) {
    let n = s.len();
    if n < 3 || !(max_accel > 0.0) || !(max_step > 0.0) {
        return;
    }
    ...
    for i in 0..n {
        if s[i] > floor[i] + 1e-12 {
            last_active = Some(i);
        }
        if let Some(l) = last_active {
            if i - l <= blend {
                mask[i] = true;
            }
        }
    ...
    let e = s.to_vec();
    for i in 1..n - 1 {
        if !mask[i] || e[i + 1] - 2.0 * e[i] + e[i - 1] >= -ad * (1.0 + 1e-6) {
            continue;
        }
        let (before, after) = (e[i] - e[i - 1], e[i + 1] - e[i]);
        let w = if before * after <= 0.0 {
            0.0
        } else if before.abs() < after.abs() {
            before
        } else {
            after
        }
        ...
        for dir in [1i64, -1] {
            ...
            loop {
                ...
                if (dir > 0 && slope < -vd) || (dir < 0 && slope > vd) {
                    break;
                }
    ...
    for pair in hull.windows(2) {
        ...
        if q <= p + 1 || !mask[p..=q].iter().any(|m| *m) {
            continue;
        }
        for j in p + 1..q {
...
pub fn compensation(
    ...
    smoothed_focal_lengths: &[Option<f64>],
    enabled: bool,
    frame: usize,
) -> f64 {
    if !enabled {
...
pub fn compensation_at(params: &ComputeParams, frame: usize) -> f64 {
    compensation(
        &params.focal_lengths,
        &params.smoothed_focal_lengths,
        params.focal_length_smoothing_enabled,
        frame,
    )
}
...
pub fn compute_base_curve(params: &ComputeParams) -> Vec<f64> {
    ...
    let removed = remove_outliers(
        &mut raw,
        (OUTLIER_MAX_SECONDS * params.scaled_fps).ceil() as usize,
    );
    if removed > 0 {
        log::warn!(
            "Focal length metadata: {removed} frames replaced as glitches (isolated excursions of more than {:.0}%)",
            (OUTLIER_LN_RATIO.exp() - 1.0) * 100.0
        );
    }
...
pub fn shift_by_delay(base: &[f64], delay_frames: i32) -> Vec<f64> {
    ...
    (0..n)
        .map(|i| base[(i + delay_frames as i64).clamp(0, n - 1) as usize])
        .collect()
}
...
pub fn derive_curves(
    ...
    (
        dequantized.into_iter().map(Some).collect(),
        target.into_iter().map(Some).collect(),
    )
}
