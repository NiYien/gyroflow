...
impl HorizonLock {
    ...
    pub fn lock(
        ...
        if self.lock_enabled || keyframes.is_keyframed(&KeyframeType::LockHorizonAmount) {
            let mut roll_rates: std::collections::BTreeMap<i64, f64> =
            ...
            if let Some(gvec) = grav {
                if !gvec.is_empty() && use_grav {
                    ...
                    for (ts, smoothed_ori) in quats.iter_mut() {
                        ...
                        let video_rotation = keyframes
                            .value_at_gyro_timestamp(&KeyframeType::VideoRotation, timestamp_ms)
                            .unwrap_or(compute_params.video_rotation);
                        let horizonroll = keyframes
                            .value_at_gyro_timestamp(&KeyframeType::LockHorizonRoll, timestamp_ms)
                            .unwrap_or(self.horizonroll)
                            + video_rotation;
                        let horizonlockpercent = keyframes
                            .value_at_gyro_timestamp(&KeyframeType::LockHorizonAmount, timestamp_ms)
                            .unwrap_or(self.horizonlockpercent);
                        ...
                        if self.automatic_lock {
                            ...
                            let target = if let Some(&roll_rate) = roll_rates.get(ts) {
                                if roll_rate.abs() > self.turn_threshold {
                                    roll_rate * self.turn_multiplier
                                } else {
                                    0.0
                                }
                            } else {
                                    ...
                                    if let Some(prev_ts_val) = prev_tilt_ts_grav {
                                        ...
                                        if dt > 0.0 {
                                            ...
                                            if change.abs() > max_change {
                                                accel_limited =
                                                    prev_tilt + change.signum() * max_change;
                                            }
                                        }
                                    }
                                }
                        ...
                        let locked_ori = smoothed_ori.to_rotation_matrix()
                            * Rotation3::from_axis_angle(
                                &z_axis,
                                -angle_corr
                                    + gv[0].simd_atan2(gv[1])
                                    + total_horizonroll_deg * std::f64::consts::PI / 180.0,
                            );
                        *smoothed_ori = UnitQuaternion::from_rotation_matrix(&locked_ori)
                            .slerp(&smoothed_ori, 1.0 - horizonlockpercent / 100.0)
                    }
                    return;
                }
            }
            ...
            for (ts, smoothed_ori) in quats.iter_mut() {
                ...
                let horizonroll = keyframes
                    ...
                    .unwrap_or(self.horizonroll)
                    + video_rotation;
                let horizonpitch = keyframes
                    .value_at_gyro_timestamp(&KeyframeType::LockHorizonPitch, timestamp_ms)
                    .unwrap_or(self.horizonpitch);
                let lock_pitch = keyframes
                    .value_at_gyro_timestamp(&KeyframeType::LockHorizonPitchEnabled, timestamp_ms)
                    .unwrap_or(if self.lock_pitch { 1.0 } else { 0.0 })
                    != 0.0;
                let horizonlockpercent = keyframes
                    .value_at_gyro_timestamp(&KeyframeType::LockHorizonAmount, timestamp_ms)
                    .unwrap_or(self.horizonlockpercent);
                ...
                *smoothed_ori = lock_horizon_angle(
                    smoothed_ori,
                    total_horizonroll_deg * std::f64::consts::PI / 180.0,
                    lock_pitch,
                    horizonpitch * std::f64::consts::PI / 180.0,
                )
                .slerp(&smoothed_ori, 1.0 - horizonlockpercent / 100.0);
            }
        }
    ...
    pub fn interpolate_gravity_vector(
        gravs: &crate::gyro_source::TimeVec,
        timestamp_us: i64,
    ) -> Option<Vector3<f64>> {
        match gravs.len() {
            0 => None,
