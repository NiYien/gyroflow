    ...
    pub fn rotate_and_distort(
        ...
        if _w > 0.0 {
            ...
            if !mesh_data.is_empty() && mesh_data[0] > 10.0 {
                ...
                let new_pos = crate::gyro_source::interpolate_mesh(
                    uv.0 as f64,
                    uv.1 as f64,
...
pub fn undistort_points_with_rolling_shutter(
    distorted: &[(f32, f32)],
    timestamp_ms: f64,
    frame: Option<usize>,
...
pub fn undistort_points(
    ...
    lens_correction_amount: f64,
    timestamp_ms: f64,
    shift_per_point: Option<Vec<(f32, f32, f32, f32, f32)>>,
    mesh: Option<Vec<f64>>,
) -> Vec<(f32, f32)> {
    ...
    distorted
        ...
        .map(|(index, pi)| {
            ...
            if params.lens.input_horizontal_stretch > 0.001 {
                x *= params.lens.input_horizontal_stretch as f32;
            }
            if params.lens.input_vertical_stretch > 0.001 {
                y *= params.lens.input_vertical_stretch as f32;
            }
            ...
            let rot = nalgebra::convert::<nalgebra::Matrix3<f64>, nalgebra::Matrix3<f32>>(
                *rot_per_point
                    .as_ref()
            ...
            if let Some(mut pt) = params.distortion_model.undistort_point(pw, &kernel_params) {
                    ...
                    && kernel_params.light_refraction_coefficient > 0.0
                {
                    ...
                    if r != 0.0 {
                        let sin_theta_d =
                            (r / (1.0 + r * r).sqrt()) / kernel_params.light_refraction_coefficient;
                        let r_d = sin_theta_d / (1.0 - sin_theta_d * sin_theta_d).sqrt();
                        let factor = r_d / r;
                        pt.0 *= factor;
                        pt.1 *= factor;
                ...
                if lens_correction_amount < 1.0 {
                    ...
                    new_pt = params.distortion_model.distort_point(
                        new_pt.0,
                        new_pt.1,
                        _w,
                        &kernel_params,
                    ...
                    if let Some(digital) = &params.digital_lens {
                        ...
                        if digital.id() == "gopro_superview"
                            || digital.id() == "gopro6_superview"
                            || digital.id() == "gopro_hyperview"
                        {
                    ...
                    pt = (
                        new_pt.0 * (1.0 - lens_correction_amount as f32)
                            + (pt.0 * lens_correction_amount as f32),
                        new_pt.1 * (1.0 - lens_correction_amount as f32)
                            + (pt.1 * lens_correction_amount as f32),
                    );
                }
                pt
            } else {
