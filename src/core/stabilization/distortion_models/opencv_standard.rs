...
impl OpenCVStandard {
    pub fn undistort_point(&self, point: (f32, f32), params: &KernelParams) -> Option<(f32, f32)> {
        ...
        for _ in 0..20 {
            let r2 = x * x + y * y;
            let icdist = (1.0 + ((params.k[7] * r2 + params.k[6]) * r2 + params.k[5]) * r2)
                / (1.0 + ((params.k[4] * r2 + params.k[1]) * r2 + params.k[0]) * r2);
            if icdist < 0.0 {
                log::warn!("icdist < 0");
                return None;
            }
            let delta_x = 2.0 * params.k[2] * x * y
                + params.k[3] * (r2 + 2.0 * x * x)
                + params.k[8] * r2
                + params.k[9] * r2 * r2;
            let delta_y = params.k[2] * (r2 + 2.0 * y * y)
                + 2.0 * params.k[3] * x * y
                + params.k[10] * r2
                + params.k[11] * r2 * r2;
            x = (x0 - delta_x) * icdist;
    ...
    pub fn distort_point(&self, x: f32, y: f32, z: f32, params: &KernelParams) -> (f32, f32) {
        ...
        let xd0 = x * cdist * icdist2
            + params.k[2] * a1
            + params.k[3] * a2
            + params.k[8] * r2
            + params.k[9] * r4;
        let yd0 = y * cdist * icdist2
            + params.k[2] * a3
            + params.k[3] * a1
            + params.k[10] * r2
            + params.k[11] * r4;
    ...
    pub fn opencl_functions(&self) -> &'static str {
        include_str!("opencv_standard.cl")
    }
    pub fn wgsl_functions(&self) -> &'static str {
        include_str!("opencv_standard.wgsl")
    }
}
