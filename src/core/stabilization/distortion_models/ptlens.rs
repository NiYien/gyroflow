...
impl PtLens {
    pub fn undistort_point(&self, point: (f32, f32), params: &KernelParams) -> Option<(f32, f32)> {
        ...
        for i in 0..10 {
            let fru = ru
                * (params.k[0] * ru * ru * ru + params.k[1] * ru * ru + params.k[2] * ru + 1.0)
                - rd;
            ...
            ru = ru
                - (fru
                    / (4.0 * params.k[0] * ru * ru * ru
                        + 3.0 * params.k[1] * ru * ru
                        + 2.0 * params.k[2] * ru
                        + 1.0));
        }
        if ru < 0.0 {
    ...
    pub fn distortion_derivative(&self, theta: f64, k: &[f64]) -> Option<f64> {
        ...
        let ru = theta;
        Some(4.0 * k[0] * ru * ru * ru + 3.0 * k[1] * ru * ru + 2.0 * k[2] * ru + 1.0)
    }
    ...
    pub fn opencl_functions(&self) -> &'static str {
        include_str!("ptlens.cl")
    }
    pub fn wgsl_functions(&self) -> &'static str {
        include_str!("ptlens.wgsl")
    }
}
