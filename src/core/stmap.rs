...
pub fn generate_stmaps(
    stab: &StabilizationManager,
    per_frame: bool,
) -> impl Iterator<Item = (String, usize, Vec<u8>, Vec<u8>)> {
    // (frame, undistort, redistort)
    let (width, height) = {
    ...
    kernel_flags.set(
        ...
        compute_params.digital_lens.is_some(),
    );
    kernel_flags.set(
    ...
    (0..compute_params.frame_count).map(move |frame| {
        ...
        compute_params.fov_scale =
            (new_width as f32 / width as f32).max(new_height as f32 / height as f32) as f64;
        compute_params.width = new_width;
        compute_params.height = new_height;
        compute_params.output_width = new_width;
        compute_params.output_height = new_height;
        ...
        let undist = parallel_exr(new_width, new_height, |x, y| {
            ...
            let mut sy = if compute_params.frame_readout_direction.is_horizontal() {
                ...
                (y.round() as i32)
                    .min(transform.kernel_params.height)
                    .max(0) as usize
            };
            if transform.kernel_params.matrix_count > 1 {
                ...
                if let Some(pt) = Stabilization::rotate_and_distort(
                    ...
                    &transform.matrices,
                    &compute_params.distortion_model,
                    compute_params.digital_lens.as_ref(),
                    r_limit_sq,
                    &mesh_data,
                ) {
                    if compute_params.frame_readout_direction.is_horizontal() {
                        sy = (pt.0.round() as i32)
                            .min(transform.kernel_params.width)
                            .max(0) as usize;
                    } else {
                        sy = (pt.1.round() as i32)
                            .min(transform.kernel_params.height)
                            .max(0) as usize;
                    }
                }
...
fn parallel_exr(
    width: usize,
    height: usize,
    cb: impl Fn(f32, f32) -> Option<(f32, f32)> + Sync,
) -> Vec<u8> {
    ...
    coords
        ...
        .for_each(|(y, row)| {
            ...
            row.chunks_mut(2).enumerate().for_each(|(x, pix)| {
