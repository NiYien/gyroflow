...
impl EstimatePoseTrait for PoseEightPoint {
    ...
    fn estimate_pose(
        ...
        use cv_core::nalgebra::{Point2, UnitVector3};

        let (pts1, pts2) = pairs.as_ref()?;

        let pts1 = crate::stabilization::undistort_points_for_optical_flow(
            &pts1,
            timestamp_us,
            params,
            size,
        );
        let pts2 = crate::stabilization::undistort_points_for_optical_flow(
            &pts2,
            next_timestamp_us,
            params,
            size,
        ...
        let matches: Vec<Match> = pts1
            .into_iter()
            .zip(pts2.into_iter())
            .map(|(i1, i2)| {
                FeatureMatch(
                    UnitVector3::new_normalize(
                        Point2::new(i1.0 as f64, i1.1 as f64).to_homogeneous(),
                    ),
                    UnitVector3::new_normalize(
                        Point2::new(i2.0 as f64, i2.1 as f64).to_homogeneous(),
                    ),
                )
            })
            .collect();
        ...
        for threshold in thresholds {
            ...
            if let Some(out) = arrsac.model(&eight_point, matches.iter().copied()) {
                ...
                return Some(nalgebra::Rotation3::from_matrix_unchecked(
                    nalgebra::Matrix3::from_column_slice(rot.matrix().as_slice()),
                ));
