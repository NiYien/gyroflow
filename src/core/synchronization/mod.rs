...
impl PoseEstimator {
    ...
    pub fn processed_frames(&self, range: Range<i64>) -> Vec<i64> {
        self.sync_results
            ...
            .iter()
            .filter_map(|x| {
                if range.contains(x.0) && x.1.rotation.is_some() {
                    Some(*x.0)
                } else {
                    None
                }
        ...
        let mut frames_to_process = Vec::new();
        {
            ...
            for (k, v) in l.iter() {
                if v.rotation.is_none() && v.frame_size.0 > 0 {
                    if let Some((next_k, _)) = l
                        .range(k..)
                        .find(|(_, next)| v.frame_no + 1 == next.frame_no && next.frame_size.0 > 0)
                    {
    ...
    pub fn filter_of_lines(lines: &OpticalFlowPairWithTs, scale: f64) -> OpticalFlowPairWithTs {
        if let Some(lines) = lines {
            ...
            lines
                ...
                .iter()
                .zip(lines.1.1.iter())
                .for_each(|(p1, p2)| sum_angles += (p2.1 - p1.1).atan2(p2.0 - p1.0));
            ...
            let (lines0, lines1) = lines
                ...
                .filter_map(|(p1, p2)| {
                    ...
                    if diff < 30.0 * (std::f32::consts::PI / 180.0) {
                        // 30 degrees
                            ...
                            if from_fr.frame_no + d == to_item.frame_no {
                                ...
                                if let Ok(mut from_of) = from_fr.optical_flow.try_borrow_mut() {
                                    from_of.insert(
                                        ...
                                        of.map(|of| {
                                            (
                                                (from_fr.timestamp_us, of.0),
                                                (to_item.timestamp_us, of.1),
                                            )
                                        }),
                                    );
                                }
    ...
    pub fn get_of_lines_for_timestamp(
        &self,
        timestamp_us: &i64,
        next_no: usize,
        scale: f64,
        num_frames: usize,
        filter: bool,
    ) -> (OpticalFlowPairWithTs, Option<(u32, u32)>) {
        ...
        for x in 0..width {
            for y in 0..height {
                ...
                img.put_pixel(
                    x,
                    y,
                    image::Rgba::from_slice(&slice[pix_pos..pix_pos + 4]).to_luma(),
                );
            }
        }
        img
    ...
    pub fn yuv_to_gray(_width: u32, height: u32, stride: u32, slice: &[u8]) -> Option<GrayImage> {
        ...
        image::GrayImage::from_raw(
            stride as u32,
            height,
            slice[0..(stride * height) as usize].to_vec(),
        )
    }
    ...
    pub fn recalculate_gyro_data(&self, fps: f64, final_pass: bool) {
        ...
        let mut update_timestamps = BTreeMap::<i64, i64>::new();
        {
            ...
            while let Some((k, v)) = iter.next() {
                ...
                if final_pass && eul.is_none() {
                    if let Some(prev_existing) =
                        sync_results.range(..*k).rev().find(|x| x.1.euler.is_some())
                    {
                        if let Some(next_existing) =
                            sync_results.range(*k..).find(|x| x.1.euler.is_some())
                        {
                            let ratio = (*k - prev_existing.0) as f64
                                / (next_existing.0 - prev_existing.0) as f64;
        ...
        if lpf > 0.0 && fps > 0.0 {
            ...
            if let Err(e) =
                crate::filtering::Lowpass::filter_gyro_forward_backward(lpf, fps, &mut vals)
            {
    ...
    pub fn find_offsets<F: Fn(f64) + Sync>(
        ...
        match self.offset_method.load(SeqCst) {
            ...
            1 => find_offset::visual_features::find_offsets(
                &self,
                ranges,
                sync_params,
                params,
