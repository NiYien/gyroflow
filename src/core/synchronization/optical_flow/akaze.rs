...
impl OFAkaze {
    pub fn detect_features(
        ...
        let (points, descriptors) = akz.extract(&image::DynamicImage::ImageLuma8(
            Arc::try_unwrap(img).unwrap(),
        ));
...
impl OpticalFlowTrait for OFAkaze {
    fn optical_flow_to(&self, to: &OpticalFlowMethod) -> OpticalFlowPair {
        if let OpticalFlowMethod::OFAkaze(to) = to {
            return Some(
                Self::match_descriptors(&self.descriptors, &to.descriptors)
                    .into_iter()
                    .map(|(i1, i2)| (self.features[i1].clone(), to.features[i2].clone()))
                    .unzip(),
            );
        }
        None
