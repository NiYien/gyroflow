...
pub fn calculate_fovs(
    ...
    if compute_params.focal_length_smoothing_enabled
        && !compute_params.smoothed_focal_lengths.is_empty()
    {
    ...
    let final_fovs = if compute_params.adaptive_zoom_window < -0.9 {
        // Static zoom
...
pub fn get_checksum(compute_params: &ComputeParams, smoothing_checksum: u64) -> u64 {
    ...
    for x in compute_params
        .focal_lengths
        .iter()
        .chain(compute_params.smoothed_focal_lengths.iter())
    {
