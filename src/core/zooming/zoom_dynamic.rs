...
pub fn compute(
                ...
                || keyframes.is_keyframed(&KeyframeType::VideoSpeed)))
    {
        ...
        let data_per_timestamp = timestamps
            .iter()
            .map(|(_frame, ts)| {
                let mut window = keyframes
                    .value_at_video_timestamp(&KeyframeType::ZoomingSpeed, *ts)
                    .unwrap_or(window);
...
fn min_rolling(a: &[f64], window: usize) -> Vec<f64> {
    a.windows(window)
        .filter_map(|window| window.iter().copied().reduce(f64::min))
        .collect()
}
...
fn min_rolling_dynamic(
    a: &[f64],
    max_window_half: isize,
    data_per_timestamp: &[DataPerTimestamp],
) -> Vec<f64> {
    ...
    for (di, data) in data_per_timestamp.iter().enumerate() {
        ...
        if i >= 0 && i as usize + data.frames <= a.len() {
            ...
            log::error!(
                "Something went wrong i: {i}, a.len: {}, frames: {}",
                a.len(),
                data.frames
            );
        }
    ...
    for (di, data) in data_per_timestamp.iter().enumerate() {
        ...
        if i >= 0 && i as usize + data.frames <= a.len() {
            ...
            if window.len() == filter.len() {
                ...
                log::error!(
                    "Something went wrong window.len: {}, filter.len: {}",
                    window.len(),
                    filter.len()
                );
            }
...
fn envelope_follower(
    ...
    let alphas = if let Some(alpha) = alpha {
        ...
        data_per_timestamp
            .iter()
            .map(|dpt| 1.0 - (-(1.0 / dpt.fps) / dpt.window).exp())
            .collect::<Vec<_>>()
    };
