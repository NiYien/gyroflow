...
pub fn map_hardware_format(format: Pixel, frame: &Video) -> Option<Pixel> {
    match format {
        ...
        Pixel::VIDEOTOOLBOX => {
            let pix_fmt = unsafe {
                mac_ffi::CVPixelBufferGetPixelFormatType(
                    (*frame.as_ptr()).data[3] as mac_ffi::CVPixelBufferRef,
                )
            };
            ...
            match &pix_fmt_bytes {
                ...
                _ => {
                    log::error!("Unknown VT pixel format: {pix_fmt:08x}");
                    None
                }
            }
        }
...
pub fn get_plane_buffer<'a>(
    frame: &'a mut Video,
    size: (usize, usize),
    plane_index: usize,
    render_globals: &mut RenderGlobals,
    wgpu_format: Option<gyroflow_core::WgpuTextureFormat>,
) -> BufferDescription<'a> {
    match frame.format() {
        #[cfg(any(target_os = "macos", target_os = "ios"))]
        Pixel::VIDEOTOOLBOX => {
            let cache = render_globals
                .tex_cache
                .get_or_insert_with(|| mac_ffi::MetalTextureCache::new().unwrap()); // TODO: unwrap

            let f =
                gyroflow_core::gpu::wgpu_interop_metal::format_wgpu_to_metal(wgpu_format.unwrap()); // TODO: unwrap
...
pub mod mac_ffi {
    ...
    unsafe extern "C" {
        ...
        pub fn CVMetalTextureCacheFlush(
            textureCache: CVMetalTextureCacheRef,
    ...
    impl Drop for TextureHandle {
        fn drop(&mut self) {
            ...
            unsafe {
                CFRelease(self.texture_ref as *const _);
            }
        }
    }
    ...
    impl Drop for MetalTextureCache {
        fn drop(&mut self) {
            unsafe {
                CFRelease(self.cv_cache as *const _);
            }
        }
    }
}
