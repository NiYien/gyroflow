...
impl TimelineGyroChart {
        ...
        if series.is_empty() {
            ...
            if self.viewMode == 3 {
                ...
                for (ts, sq) in org_smoothed_quats.iter_mut() {
                    let org_quat = GyroSource::clamped_quat_at_gyro_timestamp(
                        &gyro.quaternions,
                        *ts as f64 / 1000.0,
                    );
        ...
        if series.is_empty() || series == "8" {
            ...
            if gyro.file_metadata.read().has_per_frame_focal_length() {
                ...
                self.focal_lengths = params
                    ...
                    .collect();
                self.smoothed_focal_lengths = params
                    .smoothed_focal_lengths
                    .iter()
                    .enumerate()
                    .filter_map(|(i, v)| {
                        v.map(|x| ChartData {
    ...
    pub fn update_data(&mut self, series: &str) {
        if series.is_empty() {
            ...
            match self.viewMode {
                ...
                3 => {
                    ...
                    self.series[SERIES_SYNC_X].data =
                        Self::get_serie_vector(&self.smoothed_quats, 0);
                    self.series[SERIES_SYNC_Y].data =
                        Self::get_serie_vector(&self.smoothed_quats, 1);
                    self.series[SERIES_SYNC_Z].data =
                        Self::get_serie_vector(&self.smoothed_quats, 2);
                    self.series[SERIES_SYNC_W].data =
                        Self::get_serie_vector(&self.smoothed_quats, 3);
                }
        ...
        self.series[SERIES_FOCAL_LENGTH_SM].data =
            Self::get_serie_vector(&self.smoothed_focal_lengths, 0);
...
impl QQuickPaintedItem for TimelineGyroChart {
    fn paint(&mut self, p: &mut QPainter) {
        ...
        if self.series[SERIES_X].visible {
            self.drawAxis(p, SERIES_X, colors[0]);
        }
        if self.series[SERIES_Y].visible {
            self.drawAxis(p, SERIES_Y, colors[1]);
        }
        if self.series[SERIES_Z].visible {
            self.drawAxis(p, SERIES_Z, colors[2]);
        }
        if self.series[SERIES_W].visible {
            self.drawAxis(p, SERIES_W, colors[3]);
        }

        if self.series[SERIES_SYNC_X].visible {
            self.drawAxis(p, SERIES_SYNC_X, colors[4]);
        }
        if self.series[SERIES_SYNC_Y].visible {
            self.drawAxis(p, SERIES_SYNC_Y, colors[5]);
        }
        if self.series[SERIES_SYNC_Z].visible {
            self.drawAxis(p, SERIES_SYNC_Z, colors[6]);
        }
        if self.series[SERIES_SYNC_W].visible {
            self.drawAxis(p, SERIES_SYNC_W, colors[7]);
        }

        if self.series[SERIES_FOV].visible {
            self.drawOverlay(p, SERIES_FOV, colors[8]);
        } // zooming amount

        if self.series[SERIES_FOCAL_LENGTH].visible {
            self.drawAxis(p, SERIES_FOCAL_LENGTH, "#ff6600");
        } // raw FL
        if self.series[SERIES_FOCAL_LENGTH_SM].visible {
            self.drawAxis(p, SERIES_FOCAL_LENGTH_SM, "#cccc66");
        } // smoothed FL (drawn on top)

        if self.series[SERIES_SYNC_POINTS].visible {
