    ...
    E: ElementConversion,
{
    ...
    // niyien fix: pass `(&device, dtype)` so the padded tensor inherits the
    // input dtype. Without this, F16 inputs get padded with an F32-default
    // tensor and slice_assign panics with `DTypeMismatch` in burn-ir.
    let padded_tensor =
        Tensor::full(padded_dims, value, (&tensor.device(), tensor.dtype()));
