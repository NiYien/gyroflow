...
use neuflow_trace_shared::{self as neuflow_trace, CounterMetric, DurationMetric, TracePhase};
use std::sync::Arc;
    ...
    R: FusionRuntime + 'static,
{
    ...
    {
        let trace_phase = neuflow_trace::current_phase();
        let trace_seq = neuflow_trace::current_seq();
        let trace_start = Instant::now();
        let output_alloc_start = Instant::now();
        ...
        let outputs = repr
            ...
            .collect();
        let output_alloc_elapsed = output_alloc_start.elapsed();
        ...
        if trace_phase == TracePhase::Forward && let Some(seq) = trace_seq {
            neuflow_trace::record_counter(seq, CounterMetric::ForwardRegisterCall, 1);
            neuflow_trace::record_duration(
                seq,
                DurationMetric::ForwardClientSubmit,
                trace_start.elapsed(),
            );
            neuflow_trace::record_duration(
                seq,
                DurationMetric::ForwardClientOutputAlloc,
                output_alloc_elapsed,
            );
            neuflow_trace::record_duration(
                seq,
                DurationMetric::ForwardClientEnqueue,
                enqueue_elapsed,
            );
        }
