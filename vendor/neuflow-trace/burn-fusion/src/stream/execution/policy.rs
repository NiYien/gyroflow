...
use neuflow_trace_shared::{self as neuflow_trace, CounterMetric};
...
impl<O: core::fmt::Debug> Policy<O> {
    ...
    pub fn action(
        ...
        if let Some((id, _length)) = self.found {
            neuflow_trace::record_counter_current(CounterMetric::ForwardPlanHit, 1);
        ...
        match action {
            Action::Explore => {
                neuflow_trace::record_counter_current(CounterMetric::ForwardPlanMiss, 1);
            }
            Action::Defer => {
                neuflow_trace::record_counter_current(CounterMetric::ForwardActionDefer, 1);
            }
            Action::Execute(_) => {
                neuflow_trace::record_counter_current(CounterMetric::ForwardPlanHit, 1);
            }
