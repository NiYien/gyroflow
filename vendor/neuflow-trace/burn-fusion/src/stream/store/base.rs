...
use neuflow_trace_shared::{self as neuflow_trace, CounterMetric, DurationMetric};
...
impl<O: core::fmt::Debug> ExecutionPlanStore<O> {
    ...
    pub fn find(&self, query: SearchQuery<'_>) -> Vec<ExecutionPlanId> {
        ...
        let result = self.index.find(query);
        neuflow_trace::record_duration_current(DurationMetric::ForwardPlanFind, trace_start.elapsed());
        result
    }
    ...
    pub fn add(&mut self, exploration: ExecutionPlan<O>) -> ExecutionPlanId {
        ...
        neuflow_trace::record_counter_current(CounterMetric::ForwardPlanAdd, 1);
