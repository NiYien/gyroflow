...
use neuflow_trace_shared::{self as neuflow_trace, DurationMetric, TracePhase, enter_context};
use std::{
    any::{Any, TypeId},
    boxed::Box,
    cell::RefCell,
...
impl<S: DeviceService + 'static> DeviceHandleSpec<S> for ChannelDeviceHandle<S> {
    ...
    fn submit_blocking<R: Send + 'static, T: FnOnce(&mut S) -> R + Send + 'static>(
        ...
        self.submit_inner::<_, SEND_FLUSH>(move |state: &mut S| {
            let _trace_guard = enter_context(trace_context);
    ...
    fn submit<T: FnOnce(&mut S) + Send + 'static>(&self, task: T) {
        let trace_context = neuflow_trace::capture_context();
        self.submit_inner::<_, SEND_NO_FLUSH>(move |state| {
            let _trace_guard = enter_context(trace_context);
            task(state);
        })
...
impl<S: DeviceService + 'static> ChannelDeviceHandle<S> {
    ...
    fn submit_inner<T: FnOnce(&mut S) + Send + 'static, const FLUSH: bool>(
        ...
        let func_init = move || {
            ...
            if trace_context.phase == TracePhase::Forward
                && let Some(seq) = trace_context.seq
            {
                neuflow_trace::record_duration(
                    seq,
                    DurationMetric::ForwardRunnerTask,
                    task_start.elapsed(),
                );
            }
        };
...
mod custom_channel {
    ...
    pub const CHANNEL_MAX_TASK: usize = 256;
    /// Start processing before the buffer is completely full to reduce latency
    /// while still keeping enough work for the server thread.
    const AUTO_FLUSH_THRESHOLD: usize = CHANNEL_MAX_TASK * 3 / 4;
    ...
    impl DeviceClient {
        ...
        pub fn enqueue<F: FnOnce() -> TaskResult + Send + 'static>(
            ...
            loop {
                ...
                if spun
                    && trace_context.phase == TracePhase::Forward
                    && let Some(seq) = trace_context.seq
                {
                    neuflow_trace::record_duration(
                        seq,
                        DurationMetric::ForwardClientBackpressure,
                        spin_start.elapsed(),
                    );
                }
                ...
                // Kick the server thread once the queue is mostly full instead of waiting for a
                // completely full buffer. This keeps batching benefits while reducing the time
                // tasks spend queued before execution starts.
                if index + 1 >= AUTO_FLUSH_THRESHOLD {
                    self.flush();
                }
