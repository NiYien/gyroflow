...
impl<R: Runtime> ComputeClient<R> {
    ...
    pub fn read_one_tensor_async(
        ...
        let trace_context = neuflow_trace::capture_context();
        let fallback_client = self.clone();
        let profile_client = self.clone();
        let read_client = self.clone();

        async move {
            let _trace_guard = neuflow_trace::enter_context(trace_context);

            if trace_context.phase != TracePhase::Readback || trace_context.seq.is_none() {
                let fut = fallback_client.read_tensor_async(vec![descriptor]);
                return Ok(fut.await?.remove(0));
            }

            let seq = trace_context.seq.expect("checked above");
            let trace_start = Instant::now();
            let (result, profile) = profile_client
                .profile(
                    move || cubecl_common::reader::read_sync(read_client.read_tensor_async(vec![descriptor])),
                    "neuflow-readback",
                )
                .map_err(|err| ServerError::Generic {
                    reason: format!("Failed to profile NeuFlow readback: {err:?}"),
                    backtrace: BackTrace::capture(),
                })?;

            let mut result = result.map_err(|err| ServerError::Generic {
                    reason: format!("NeuFlow readback failed during profile: {err:?}"),
                    backtrace: BackTrace::capture(),
                })?;

            neuflow_trace::record_duration(seq, DurationMetric::ReadbackTotal, trace_start.elapsed());

            let gpu_profile_ms = if profile.timing_method() == TimingMethod::Device {
                Some(profile.resolve().await.duration().as_secs_f64() * 1000.0)
            } else {
                None
            };
            neuflow_trace::set_gpu_profile_ms(seq, gpu_profile_ms);
