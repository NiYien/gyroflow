...
use cubecl_common::{
    backtrace::BackTrace,
    bytes::Bytes,
    profile::{Instant, ProfileDuration, TimingMethod},
};
use cubecl_core::{
...
impl WgpuStream {
    ...
    pub fn read_resources(
        ...
        if trace_context.phase == TracePhase::Readback && let Some(seq) = trace_context.seq {
            neuflow_trace::record_duration(
                seq,
                DurationMetric::ReadbackCopyToStaging,
                copy_start.elapsed(),
            );
        }
        ...
        if trace_context.phase == TracePhase::Readback && let Some(seq) = trace_context.seq {
            neuflow_trace::record_duration(
                seq,
                DurationMetric::ReadbackFlushSubmit,
                flush_start.elapsed(),
            );
        }
        ...
        Box::pin(async move {
            ...
            neuflow_trace::record_duration_current(
                DurationMetric::ReadbackMapWait,
                map_wait_start.elapsed(),
            );
